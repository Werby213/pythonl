...
class RenderGL:
    ...
    def doDraw(self, w, h, xoff, yoff):
        ...
        gl.glTexCoord2f(0 + xoff, 1 + yoff);
        gl.glVertex3f(0, 1, self.z)
        gl.glTexCoord2f(0 + xoff, 0 + yoff);
        gl.glVertex3f(0, 0, self.z)
        gl.glTexCoord2f(1 + xoff, 0 + yoff);
        gl.glVertex3f(1, 0, self.z)
        gl.glTexCoord2f(1 + xoff, 1 + yoff);
        gl.glVertex3f(1, 1, self.z)
        gl.glEnd()
...
class CL:
    ...
    def initBuffers(self):
        ...
        self.render_buf = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR,
                                    hostbuf=np.zeros((self.ar_ySize, self.ar_ySize), dtype=np.int32))
    ...
    def execute(self):
        if self.tickState == False:
            self.kAutomata.RunAutomata(self.queue, self.a.shape, None, self.ar_ySize, self.a_buf,
                                       self.b_buf)  # np.int32(self.offset),
        else:
    ...
    def bitRender(self, rn, zoom, CLMAXVAL, colour_mode):
        ...
        if colour_mode == 1:
            RGB_array[:, :, 0] = np.where(MainCL.a != 0,
                                          np.where(MainCL.a < CLMAXVAL, 255 - (MainCL.a * 200) / CLMAXVAL, 55), 0)
            RGB_array[:, :, 1] = np.where(MainCL.a != 0,
                                          np.where(MainCL.a < CLMAXVAL, 255 - (MainCL.a * 200) / CLMAXVAL, 55), 0)
            RGB_array[:, :, 2] = np.where(MainCL.a != 0,
                                          np.where(MainCL.a < CLMAXVAL, 255 - (MainCL.a * 200) / CLMAXVAL, 55), 0)
        ...
        if colour_mode == 3:
            RGB_array[:, :, 0] = np.where(MainCL.a == 1, 255, np.where(MainCL.a != 0, np.where(MainCL.a < 50, 255 - (
                        MainCL.a * 255) / (50), 55), 0))
            RGB_array[:, :, 1] = np.where(MainCL.a == 1, 255,
                                          np.where(MainCL.a != 0, np.where(MainCL.a < 8, (MainCL.a * 255) / (8), 55),
                                                   0))
            RGB_array[:, :, 2] = np.where(MainCL.a == 1, 255,
                                          np.where(MainCL.a != 0, np.where(MainCL.a < 128, (MainCL.a * 255) / 128, 255),
                                                   0))

        if colour_mode == 4:
            RGB_array[:, :, 0] = np.where(MainCL.a == 1, 255, np.where(MainCL.a != 0, np.where(MainCL.a < 50, 255 - (
                        MainCL.a * 255) / (50), np.where(MainCL.a > 355, (MainCL.a / 3) % 256, 0)), 0))
            RGB_array[:, :, 1] = np.where(MainCL.a == 1, 255, np.where(MainCL.a != 0,
                                                                       np.where(MainCL.a < 8, (MainCL.a * 255) / (8),
                                                                                np.where(MainCL.a > 127,
                                                                                         (MainCL.a) % 256, 55)), 0))
            RGB_array[:, :, 2] = np.where(MainCL.a == 1, 255, np.where(MainCL.a != 0,
                                                                       np.where(MainCL.a < 128, (MainCL.a * 255) / 128,
                                                                                np.where(MainCL.a > 127,
                                                                                         255 - ((MainCL.a / 3) % 256),
                                                                                         55)), 0))
        ...
        if colour_mode == 5:
            ...
            RGB_array[:, :, 1] = np.where(MainCL.a == 1, 255, np.where(MainCL.a != 0,
                                                                       np.where(MainCL.a < 8, (MainCL.a * 255) / (8),
                                                                                np.where(MainCL.a > 99919,
                                                                                         255 - ((MainCL.a / 3) % 256),
                                                                                         np.where(MainCL.a > 255,
                                                                                                  255 - ((
                                                                                                                     MainCL.a / 5) % 256),
                                                                                                  np.where(
                                                                                                      MainCL.a > 63, ((
...
if __name__ == "__main__":
    ...
    if (not vetoConfig):
        uinput = raw_input(
            "  > (Int) [2 ^ " + str(res_expo) + " = " + str(2 ** res_expo) + "] Resolution: 2 to the power of: ")
        if uinput != "":
            if int(uinput) < 16:
                res_expo = int(uinput)
            else:
                print
                "2 ^", int(
                    uinput), "resolution is too large. 2^16 = ", 2 ** 16, "x", 2 ** 16, "pixels. You may disable this limit manually, just above this message in main.py"
                res_expo = 9
    ...
    if (not vetoConfig):
        ...
        if usePreset == "N" or usePreset == "n":
            ...
            if SeedType != "" and SeedType != "n" and SeedType != "N":
                # Seed from image
                seed_bitmap_image = tfd.askopenfilename(initialdir="./SeedImages",
                                                        title="Select Seeding-Image File (*.bmp)")
                MainCL.loadImg(seed_bitmap_image)
            else:
                # Seed Strength
        ...
        if not paused:
            ...
            if (show_ship and use_filter):
                ...
                if (bullet_life > 0):
                    ...
                    bullet_x -= bullet_xv
                    bullet_y -= bullet_yv
                    MainCL.place_square_GPU(np.int32(bullet_x + ((ship_size / 2) - (bullet_size / 2))),
                                            np.int32(bullet_y + ((ship_size / 2) - (bullet_size / 2))),
                                            np.int32(bullet_size))
                    MainCL.place_square_filter_GPU(np.int32(bullet_x + ((ship_size / 2) - (bullet_size / 2))),
                                                   np.int32(bullet_y + ((ship_size / 2) - (bullet_size / 2))),
                                                   np.int32(bullet_size))
