...
def open_console_window():
    global output, error, output_text
    ...
    btn_start_code = Button(console_window, text='Выполнить код', height=1, width=12, fg='black', command=run_code)
    btn_start_code.place(x=10, y=10)
    btn_start_code.bind('<KeyRelease>', lambda event: refresh_console())
...
text1 = Text(frame, font=font  + str(font_size), fg="white", bg="black", wrap=WORD)
text1.pack(fill="both", expand=True, padx=0, pady=(82, 0))
...
word_count()
btn_start_code = Button(frame, text='Выполнить код', height=1, width=12, fg='black', command=run_code)
btn_start_code.place(x=600, y=50)
select_programming_lang = ttk.Combobox(frame, values=programming_language)
select_programming_lang.current(0)
select_programming_lang.place(x=450, y=5)
btn_console = Button(frame, text="Python консоль", command=open_console_window)
btn_console.place(x=600, y=20)
root.update_idletasks()
run_code()
text1.bind('<KeyRelease>', lambda event: word_count())
btn_start_code.bind('<KeyRelease>', lambda event: run_code())
open_console_window()
root.mainloop()